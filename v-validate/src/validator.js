const validateFns = {
  ...
  password(key, val) {
    const regExp = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&\(\)])[A-Za-z\d$@$!%*#?&\(\)]{6,}$/i; // IE RegExp 객체 이슈로 형태 변경
  ...
  name(key, val, option) {
    ...
    const regExp = /^([^\<\>ㄱ-ㅣ]|[\w]){${Number(option[0])},${Number(option[1])}}$/;
    return regExp.test(val);
  },
  id(key, val, option) {
    if (!option) return 'option(길이)이 설정되지 않았습니다.';
    if (option.length < 2) return 'option(길이) 값은 2개 여야 합니다.';
    const regExp = /^([-_a-z\d]){{Number(option[0])},${Number(option[1])}}$/;
    return regExp.test(val);
  },
};
